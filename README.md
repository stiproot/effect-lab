...
A fiber is created any time an effect is run. 
When running effects concurrently, a fiber is created for each concurrent effect.
...
The format of the Fiber data structure:
```txt
        ┌─── Represents the success type
        │        ┌─── Represents the error type
        │        │
